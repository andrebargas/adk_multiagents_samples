...
from data_explorer_agent.sub_agents.sql_explorer.chase_sql import chase_constants

from google.adk.tools import ToolContext
from google.genai import Client

project_id = get_env_var("GOOGLE_CLOUD_PROJECT")
bq_project = get_env_var("BQ_PROJECT_ID", None)
region = get_env_var("GOOGLE_CLOUD_REGION", "us-central1")
llm_client = Client(vertexai=True, project=project_id, location=region)
...
def get_bigquery_schema(bq_dataset_id, bq_project_id, client=None, project_id=None):
...
def run_bigquery_validation(
    ...
    try:
        ...
        if results.schema:  # Check if query returned data
            ...
            final_result["query_result"] = json.dumps(rows)
